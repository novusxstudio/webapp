/**
 * LookaheadBotV6 - Extends V5 with additive range influence instead of multiplicative.
 * 
 * Key Change from V5:
 * - Range no longer multiplies the distance score
 * - Instead, enemy attack range contributes a small additive bias
 * - rangeBias = RANGE_BIAS * E.attackRange
 * - Enemy counters friendly: score -= base, score -= rangeBias
 * - Friendly counters enemy: score += base, score += rangeBias
 * - Equal matchups: unchanged (no range bias)
 * 
 * All other scoring remains identical to V5.
 */
...
/**
 * V6: Additive range bias (replaces multiplicative RANGE_FACTOR from V5)
 * rangeBias = RANGE_BIAS * attackRange
 * 
 * Must be << smallest attack bonus (2.0)
 * Must be << control-point occupancy bonus (2.0)
 */
...
/**
 * V6: Calculate additive range bias for an enemy unit
 * rangeBias = RANGE_BIAS * attackRange
 */
...
// ============================================================================
// Side Scoring Function (V6 - Additive Range Influence)
// ============================================================================

/**
 * Score the game state from one side's perspective.
 * 
 * V6 Change: Range influence is now additive, not multiplicative.
 * - rangeBias = RANGE_BIAS * enemy.attackRange
 * - Counter relationships: score += base + rangeBias (or -= for threats)
 * - Equal matchups: no range bias
 * 
...
function scoreSide(
  ...
  // =========================================================================
  // 5️⃣ Distance-Based Scoring (V6 ADDITIVE RANGE)
  // 
  // For counter relationships, apply ADDITIVE range bias:
  //   base = WEIGHT * exp(-ALPHA * distance)
  //   rangeBias = RANGE_BIAS * opposing.attackRange
  // 
  //   - Side counters opposing: score += base, score += rangeBias
  //   - Opposing counters side: score -= base, score -= rangeBias
  //   - Equal matchups: no range bias (unchanged from V4)
  // =========================================================================
  for (const sideUnit of sideUnits) {
    const sideType = sideUnit.unit.stats.type;
    const countersTypes = getCounteredUnits(sideType);
    ...
    for (const opposingUnit of opposingUnits) {
      ...
      // V6: Get ADDITIVE range bias from opposing unit's attack range
      const rangeBias = getRangeBias(opposingUnit.unit.stats.attackRange);
      
      // 1️⃣ Side counters opposing (POSITIVE: base + rangeBias)
      // Getting close to ranged prey is slightly more valuable
      if (countersTypes.includes(opposingType)) {
        score += base;
        score += rangeBias;
      }
      // 2️⃣ Opposing counters side (NEGATIVE: base + rangeBias)
      // Ranged enemies add a small extra threat
      else if (counteredByTypes.includes(opposingType)) {
        score -= base;
        score -= rangeBias;
      }
      // 3️⃣ Equal matchups (NO range bias, unchanged from V4)
...
// ============================================================================
// Game State Scoring Function (UNCHANGED from V5)
// ============================================================================
...
// ============================================================================
// Action Selection (UNCHANGED from V5)
// ============================================================================
...
export function createLookaheadBotV6(): Bot {
  return {
    id: 'lookahead_bot_v6',
    name: 'Lookahead(v6)',
