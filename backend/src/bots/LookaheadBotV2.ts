/**
 * LookaheadBotV2 - A zero-sum, symmetric scoring bot using one-step lookahead.
 * 
 * Core Design:
 * - scoreSide(gameState, side) computes ONLY positive values for that side
 * - scoreGameState(gameState, playerId) = myScore - enemyScore
 * - Enemy scoring is an exact mirror of friendly scoring
 * 
 * NO hardcoded negatives inside scoreSide.
 * NO special-casing enemy logic.
 * NO mixing friendly and enemy logic in the same loop.
 */
...
/** Small weight factor for distance-based scoring */
...
function scoreSide(
  ...
  // =========================================================================
  // 4️⃣ Defensive Risk (scored as POSITIVE here, subtraction makes it negative)
  ...
  // =========================================================================
  // 5️⃣ Distance-Based Scoring (Small Weights)
  // Small positive score inversely proportional to distance to:
  // - Opposing units this side counters
  // - Control points
  // =========================================================================
  for (const sideUnit of sideUnits) {
    const sideType = sideUnit.unit.stats.type;
    ...
    // Distance to opposing units we counter
    for (const opposingUnit of opposingUnits) {
...
export function createLookaheadBotV2(): Bot {
  return {
    id: 'lookahead_bot_v2',
    name: 'Lookahead(v2)',
