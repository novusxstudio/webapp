/**
 * LookaheadBotV5 - Extends V4 with range-aware distance polarity.
 * 
 * Key Change from V4:
 * - Distance scoring involving enemy units is amplified by the enemy's attack range
 * - rangeMultiplier = 1 + RANGE_FACTOR * E.attackRange
 * - Enemy counters friendly: score -= base * rangeMultiplier (ranged enemies more dangerous)
 * - Friendly counters enemy: score += base * rangeMultiplier (catching ranged enemies more valuable)
 * - Equal matchups: unchanged from V4 (no range amplification)
 * 
 * All other scoring remains identical to V4.
 */
 ...
 */
const DIST_ALPHA = 0.7;

/**
 * V5: Range amplification factor
 * rangeMultiplier = 1 + RANGE_FACTOR * attackRange
 */
const RANGE_FACTOR = 0.25;
...
/**
 * Exponential distance scoring function (base, no range amplification)
...
/**
 * V5: Calculate range multiplier for an enemy unit
 * rangeMultiplier = 1 + RANGE_FACTOR * attackRange
 */
function getRangeMultiplier(attackRange: number): number {
  return 1 + RANGE_FACTOR * attackRange;
}
...
// ============================================================================
// Side Scoring Function (V5 - Range-Aware Distance Polarity)
// ============================================================================

/**
 * Score the game state from one side's perspective.
 * Returns ONLY positive values (except for V5 range-aware counter penalties).
 * 
 * V5 Change: Distance scoring amplified by enemy attack range for counter relationships.
 * 
...
function scoreSide(
  ...
  // =========================================================================
  // 1️⃣ Units on Board: +1 per unit belonging to side (UNCHANGED)
  // =========================================================================
  ...
  // =========================================================================
  // 5️⃣ Distance-Based Scoring (V5 RANGE-AWARE)
  // 
  // For counter relationships, apply range multiplier based on opposing unit's range:
  //   - Side counters opposing: POSITIVE score (amplified by opposing range)
  //   - Opposing counters side: NEGATIVE score (amplified by opposing range)
  //   - Equal matchups: no range amplification (unchanged from V4)
  // 
  // rangeMultiplier = 1 + RANGE_FACTOR * opposingUnit.attackRange
  // =========================================================================
  for (const sideUnit of sideUnits) {
    const sideType = sideUnit.unit.stats.type;
    const countersTypes = getCounteredUnits(sideType);
    ...
    for (const opposingUnit of opposingUnits) {
      ...
      // Get base exponential distance score
      const base = expDistanceScore(ATTACK_PROX_WEIGHT, dist);
      
      // V5: Get range multiplier from opposing unit's attack range
      const rangeMultiplier = getRangeMultiplier(opposingUnit.unit.stats.attackRange);
      
      // 1️⃣ Side counters opposing (POSITIVE, amplified by opposing range)
      // Catching ranged enemies is more valuable
      if (countersTypes.includes(opposingType)) {
        score += base * rangeMultiplier;
      }
      // 2️⃣ Opposing counters side (NEGATIVE, amplified by opposing range)
      // Ranged enemies projecting counters are more dangerous
      else if (counteredByTypes.includes(opposingType)) {
        score -= base * rangeMultiplier;
      }
      // 3️⃣ Equal matchups (NO range amplification, unchanged from V4)
      else if (isEqualMatchup(sideType, opposingType)) {
        score += expDistanceScore(COUNTER_DIST_WEIGHT, dist);
      }
    }
    
    // Distance to control points (unchanged from V4, no range amplification)
...
// ============================================================================
// Game State Scoring Function (UNCHANGED from V4)
// ============================================================================
...
// ============================================================================
// Action Selection (UNCHANGED from V4)
// ============================================================================
...
export function createLookaheadBotV5(): Bot {
  return {
    id: 'lookahead_bot_v5',
    name: 'Lookahead(v5)',
