/**
 * LookaheadBotV4 - Extends V3 with exponential distance scoring.
 * 
 * Key Change from V3:
 * - All distance-based scoring now uses exponential decay: WEIGHT * exp(-ALPHA * distance)
 * - This creates commitment pressure: close positions are heavily favored, far positions negligible
 * - Approximates inevitability without multi-turn search
 * 
 * All other scoring remains identical to V3.
 */
 ...
 * At ALPHA=0.7: exp(-0.7*1)≈0.50, exp(-0.7*2)≈0.25, exp(-0.7*3)≈0.12
 */
 ...
 * 
 * Properties:
 * - At distance 0: returns WEIGHT
 * - As distance → ∞: returns → 0
 * - Decay rate controlled by DIST_ALPHA
 */
...
// ============================================================================
// Side Scoring Function (V4 - Exponential Distance Scoring)
// ============================================================================
 ...
 * 
 * V4 Change: All distance scoring uses exponential decay.
 * 
...
function scoreSide(
  ...
  // =========================================================================
  // 5️⃣ Distance-Based Scoring (V4 EXPONENTIAL)
  // 
  // All distance terms now use: WEIGHT * exp(-ALPHA * distance)
  // 
  // Weight hierarchy (preserved):
  //   ATTACK_PROX_WEIGHT (0.5) > COUNTER_DIST_WEIGHT (0.2) > CONTROL_POINT_WEIGHT (0.15)
  // =========================================================================
  for (const sideUnit of sideUnits) {
    const sideType = sideUnit.unit.stats.type;
    const countersTypes = getCounteredUnits(sideType);
    ...
    for (const opposingUnit of opposingUnits) {
      ...
      if (!inAttackRange) {
        // Can we defeat this enemy? (highest weight)
        if (canDefeat(sideType, opposingType)) {
          score += expDistanceScore(ATTACK_PROX_WEIGHT, dist);
        }
        // Do we counter this enemy? (medium weight)
        // Note: canDefeat uses counters, but we check explicitly for clarity
        else if (countersTypes.includes(opposingType)) {
          score += expDistanceScore(COUNTER_DIST_WEIGHT, dist);
        }
      }
    }
    
    // Distance to control points (lowest weight, exponential)
    for (const cp of controlPoints) {
      const dist = manhattanDistance(sideUnit.pos, cp);
      // Only add bonus if not already on the control point
      if (dist > 0) {
...
// ============================================================================
// Game State Scoring Function (UNCHANGED from V3)
// ============================================================================
...
// ============================================================================
// Action Selection (UNCHANGED from V3)
// ============================================================================
...
export function createLookaheadBotV4(): Bot {
  return {
    id: 'lookahead_bot_v4',
    name: 'Lookahead(v4)',
